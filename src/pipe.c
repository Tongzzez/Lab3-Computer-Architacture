...
uint32_t read_data_cache(uint32_t address){
    ...
    //printf("set = %d, input tag = %d\n", set, tag);

    for(way=0; way<8; way++){
        //printf("way = %d, tag = %d, age = %d\n", way, data_cache_table[set][way].tag, data_cache_table[set][way].age);
        if(data_cache_table[set][way].tag == tag && data_cache_table[set][way].age != 0){ //cache hit
            //update age
            //printf("read data cache HIT\n");
...
uint32_t read_instruction_cache(uint32_t address){ //address is pc
    ...
    for(way=0; way<4; way++){
        //printf("way = %d, tag = %d, age = %d\n", way, instruction_cache_table[set][way].tag, instruction_cache_table[set][way].age);
...
void pipe_stage_fetch()
{
    ...
    // printf("tag = %x, pc = %x\n", BTB[BTB2entry].tag, pipe.PC );
    // printf("valid = %d\n", BTB[BTB2entry].valid);
    // printf("BTB2entry = %d\n", BTB2entry);
    // printf("PHT2entry = %d\n", PHT2entry);
    // printf("PHTcounter = %d\n", PHT[PHT2entry].counter);
    // printf("PHT2entry = GHR ^ GHRpc\n");
    // printf("%x = %x ^ %x\n", PHT2entry, GHR, GHRpc);
    

    if ((BTB[BTB2entry].tag != op->pc) || BTB[BTB2entry].valid == 0 || PHT[PHT2entry].counter < 2){ // not taken
        
        //printf("predict NOT taken\n\n");
        ...
        //printf("predict TAKEN to %x\n\n", BTB[BTB2entry].target);
